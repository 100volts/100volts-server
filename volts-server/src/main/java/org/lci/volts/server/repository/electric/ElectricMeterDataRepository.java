...
public interface ElectricMeterDataRepository  extends JpaRepository<ElectricMeterData, Long> {
    ...
    Optional<Set<ElectricMeterData>> findAvrElMetersData(final int address, final String companyName,final int limit);
    ...
    @Query("SELECT u from ElectricMeterData u where u.meter.address= ?1 AND u.meter.company.name= ?2 AND u.date BETWEEN ?3 AND ?4 ORDER BY u.date desc")
    Optional<List<ElectricMeterData>> findByCompanyAdrDate(final int address, final String companyName, LocalDateTime startOfYesterday, LocalDateTime endOfYesterday);
